...
export function parseLibvipsAdapterRecipe({ buildSh, dockerfile, mesonBuild }) {
  ...
  if (!String(buildSh).includes("kleisauke:wasm-vips-$VERSION_VIPS.patch")) throw new Error("wasm-vips build.sh no longer exposes the expected libvips compatibility branch");
  if (!String(dockerfile).includes(`kleisauke:wasm-vips-${emsdkVersion}.patch`)) throw new Error("wasm-vips Dockerfile no longer exposes the expected Emscripten compatibility branch");
...
export async function resolveLibvipsAdapterBundle(version, githubJson) {
  if (!SEMVER3.test(version || "")) return { ready: false, reason: `unsupported libvips version: ${version || "<missing>"}` };
  try {
    ...
    if (recipe.libvipsVersion !== version) {
      return { ready: false, reason: `wasm-vips master ${adapter.sha.slice(0, 12)} targets libvips ${recipe.libvipsVersion}, not ${version}`, adapterCommit: adapter.sha, adapterVersion: recipe.adapterVersion, emsdkVersion: recipe.emsdkVersion };
    }
    ...
    for (const [label, sha] of [["Emscripten", emscripten?.sha], ["libvips compatibility patch", libvipsPatch?.sha], ["Emscripten compatibility patch", emscriptenPatch?.sha]]) {
      if (!SHA40.test(sha || "")) throw new Error(`${label} did not resolve to an exact commit`);
    }
    return { ready: true, reason: "adapter and compatibility patch bundle resolved to immutable commits", adapterCommit: adapter.sha, adapterVersion: recipe.adapterVersion, emsdkVersion: recipe.emsdkVersion, emscriptenRef: recipe.emsdkVersion, emscriptenCommit: emscripten.sha, libvipsPatchRef, libvipsPatchCommit: libvipsPatch.sha, emscriptenPatchRef, emscriptenPatchCommit: emscriptenPatch.sha };
  } catch (error) {
