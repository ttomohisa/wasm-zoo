...
const buildSh = [
  "VERSION_VIPS=8.19.0 # upstream",
  "curl -Ls https://github.com/libvips/libvips/compare/v$VERSION_VIPS...kleisauke:wasm-vips-$VERSION_VIPS.patch | patch -p1"
].join("\n");
const dockerfile = [
  "FROM docker.io/emscripten/emsdk:6.0.10",
  "RUN curl -Ls https://github.com/emscripten-core/emscripten/compare/6.0.10...kleisauke:wasm-vips-6.0.10.patch | patch -p1"
].join("\n");
const mesonBuild = [
  "project('wasm-vips', 'cpp',",
  "    version: '0.0.19',",
  ")",
  "vips_dep = dependency('vips', version: '>=8.19.0')"
].join("\n");

test("parse adapter recipe", () => {
  assert.deepEqual(parseLibvipsAdapterRecipe({ buildSh, dockerfile, mesonBuild }), {
    libvipsVersion: "8.19.0",
    emsdkVersion: "6.0.10",
    adapterVersion: "0.0.19",
    minimumLibvipsVersion: "8.19.0"
  });
});
...
test("resolver freezes moving refs", async () => {
  const shas = {
    adapter: "a".repeat(40),
    emscripten: "b".repeat(40),
    vipsPatch: "c".repeat(40),
    emPatch: "d".repeat(40)
  };
