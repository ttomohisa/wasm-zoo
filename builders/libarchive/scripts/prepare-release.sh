...
for file in manifest.json features.json libarchive-config.txt browser-libarchive.js BUILDINFO.txt LICENSE-libarchive.txt; do [[ -s "$dist/$file" ]] || { echo "Missing $dist/$file" >&2; exit 1; }; done
...
for file in manifest.json features.json libarchive-config.txt browser-libarchive.js BUILDINFO.txt; do cp "$dist/$file" "$stage/"; done
