...
$args=@('buildx','build','--file',(Join-Path $Root 'docker\Dockerfile'),'--target','export','--build-arg',"BUILDER_VERSION=$($envs.BUILDER_VERSION)",'--build-arg',"EMSDK_VERSION=$($envs.EMSDK_VERSION)",'--build-arg',"EMSCRIPTEN_COMMIT=$($envs.EMSCRIPTEN_COMMIT)",'--build-arg',"LIBARCHIVE_REPOSITORY=$($envs.LIBARCHIVE_REPOSITORY)",'--build-arg',"LIBARCHIVE_REF=$($envs.LIBARCHIVE_REF)",'--build-arg',"LIBARCHIVE_COMMIT=$($envs.LIBARCHIVE_COMMIT)",'--build-arg',"PROFILE=$Profile",'--output',"type=local,dest=$out",$Root)
Write-Host "WASM Zoo / libarchive $Profile" -ForegroundColor Cyan
& docker @args
