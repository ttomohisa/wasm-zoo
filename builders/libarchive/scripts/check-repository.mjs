...
const pkg=JSON.parse(fs.readFileSync(path.join(root,'../../packages/libarchive/package.json'),'utf8'));
...
need(env.includes(`LIBARCHIVE_REF=${pkg.upstream.ref}`),`libarchive must pin reviewed ref ${pkg.upstream.ref}`);
const libarchiveCommit=env.match(/^LIBARCHIVE_COMMIT=([0-9a-f]{40})$/m)?.[1];
need(Boolean(libarchiveCommit),'libarchive exact 40-character commit pin is missing');
