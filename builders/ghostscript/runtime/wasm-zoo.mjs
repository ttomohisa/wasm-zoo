// WASM Zoo Consumer API v1 ESM adapter for Ghostscript.
...
export const packageInfo = Object.freeze({
  ...
  classicScript: "browser-ghostscript.js",
...
function legacyApi() { return globalThis.WasmZooGhostscript; }

async function ensureLegacy(baseUrl) {
  if (legacyApi()) return legacyApi();
  if (typeof document === "undefined") throw new Error("Ghostscript Consumer API requires a browser document to load browser-ghostscript.js.");
  if (!classicPromise) {
    ...
    classicPromise = new Promise((resolve, reject) => {
      ...
      script.async = true;
      script.onload = () => legacyApi() ? resolve(legacyApi()) : reject(new Error("browser-ghostscript.js loaded without exposing WasmZooGhostscript."));
      script.onerror = () => reject(new Error(`Failed to load Ghostscript legacy runtime: ${scriptUrl}`));
...
function createRuntime(runner, profile) {
  ...
  return Object.freeze({
    ...
    async exec(args = [], options = {}) {
      if (disposed) throw new Error("Ghostscript Consumer API runtime has been disposed.");
