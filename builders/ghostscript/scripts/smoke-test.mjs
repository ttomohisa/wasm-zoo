...
function resolveBrowser() {
  const candidates = [process.env.GHOSTSCRIPT_WASM_BROWSER, process.env.CHROME_PATH];
  if (process.platform === "win32") {
...
async function main() {
  ...
  const timeoutMs = Number(process.env.GHOSTSCRIPT_WASM_SMOKE_TIMEOUT_MS || 240000);
  ...
  await fsp.copyFile(path.join(root, "runtime", "browser-ghostscript.js"), path.join(dist, "browser-ghostscript.js"));

  const required = [
    "smoke-test.html",
    "browser-ghostscript.js",
    "manifest.json",
    "smoke-input.pdf",
    "gs-core.js",
    "gs-core.wasm"
  ...
  if (!browser) throw new Error("Chromium browser not found. Set GHOSTSCRIPT_WASM_BROWSER to Chrome/Edge executable.");
  ...
  const temp = await fsp.mkdtemp(path.join(os.tmpdir(), "wasm-zoo-ghostscript-"));
