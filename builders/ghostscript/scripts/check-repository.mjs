...
need(/^\d+\.\d+\.\d+$/.test(pins.BUILDER_VERSION||''),'builder version must be stable semver');
need(pins.EMSDK_VERSION==='6.0.7','Emscripten must pin 6.0.7');
need(pins.EMSCRIPTEN_COMMIT==='4483d70a78098ed5d860dff2dc21f3025b2da2ee','exact Emscripten commit pin is missing');
need(/^\d+\.\d+\.\d+$/.test(pins.GHOSTSCRIPT_VERSION||''),'Ghostscript version must be stable x.y.z');
need(pins.GHOSTSCRIPT_REF===`gs${pins.GHOSTSCRIPT_VERSION}`,'Ghostscript source ref must match GHOSTSCRIPT_VERSION');
need(/^[0-9a-f]{40}$/i.test(pins.GHOSTSCRIPT_COMMIT||''),'exact Ghostscript source commit pin is missing');
const expectedReleaseTag='gs'+String(pins.GHOSTSCRIPT_VERSION||'').replaceAll('.','');
need(pins.GHOSTSCRIPT_RELEASE_TAG===expectedReleaseTag,'Ghostscript release tag must match GHOSTSCRIPT_VERSION');
const expectedSourceUrl=`https://github.com/ArtifexSoftware/ghostpdl-downloads/releases/download/${expectedReleaseTag}/ghostscript-${pins.GHOSTSCRIPT_VERSION}.tar.xz`;
need(pins.GHOSTSCRIPT_SOURCE_URL===expectedSourceUrl,'Ghostscript official source URL must match version/release tag');
need(/^[0-9a-f]{64}$/i.test(pins.GHOSTSCRIPT_SOURCE_SHA256||''),'official source archive SHA-256 is missing or invalid');
...
need(fetch.includes('GS_VERSION_MAJOR=${gs_major}') && fetch.includes('GS_VERSION_MINOR=${gs_minor}') && fetch.includes('GS_VERSION_PATCH=${gs_patch}'),'source fetch must verify version.mak dynamically');
...
for(const s of ['manifest.json','expectedVersion','-sDEVICE=png16m','-sDEVICE=pdfwrite','SMOKE_TEST_PASS_Ghostscript']) need(smoke.includes(s),`smoke contract missing: ${s}`);
...
if(errors.length){console.error(`[NG] ${errors.length} Ghostscript builder check(s)`); for(const e of errors)console.error(` - ${e}`); process.exit(1);} console.log(`[OK] Ghostscript ${pins.GHOSTSCRIPT_VERSION} browser-full repository checks passed`);
