...
version="${ZSTD_REF#v}"
[[ "$version" =~ ^([0-9]+)\.([0-9]+)\.([0-9]+)$ ]] || { echo "Invalid exact Zstandard tag version" >&2; exit 1; }
IFS='.' read -r major minor patch <<< "$version"
grep -Eq "^#define ZSTD_VERSION_MAJOR[[:space:]]+${major}$" /src/zstd/lib/zstd.h
grep -Eq "^#define ZSTD_VERSION_MINOR[[:space:]]+${minor}$" /src/zstd/lib/zstd.h
grep -Eq "^#define ZSTD_VERSION_RELEASE[[:space:]]+${patch}$" /src/zstd/lib/zstd.h
