...
function resolveBrowser() {
  const candidates = [process.env.ZSTD_WASM_BROWSER, process.env.CHROME_PATH];
  if (process.platform === "win32") {
...
async function main() {
  ...
  if (!browser) throw new Error("Chromium browser not found. Set ZSTD_WASM_BROWSER to Chrome/Edge executable.");
  ...
  const temp = await fsp.mkdtemp(path.join(os.tmpdir(), "wasm-zoo-zstd-"));
