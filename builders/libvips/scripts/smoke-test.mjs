...
function resolveBrowser() {
  const candidates = [process.env.LIBVIPS_WASM_BROWSER, process.env.CHROME_PATH];
  if (process.platform === "win32") {
...
async function main() {
  ...
  const timeoutMs = Number(process.env.LIBVIPS_WASM_SMOKE_TIMEOUT_MS || 120000);
  ...
  await fsp.copyFile(path.join(root, "runtime", "browser-libvips.js"), path.join(dist, "browser-libvips.js"));
  ...
  if (!browser) throw new Error("Chromium browser not found. Set LIBVIPS_WASM_BROWSER to Chrome/Edge executable.");
  ...
  const temp = await fsp.mkdtemp(path.join(os.tmpdir(), "wasm-zoo-libvips-"));
