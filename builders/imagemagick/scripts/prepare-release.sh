...
for file in magick-core.js magick-core.wasm magick-core.js.gz magick-core.wasm.gz manifest.json features.json imagemagick-config.txt browser-imagemagick.js BUILDINFO.txt LICENSE-imagemagick.txt; do [[ -s "$dist/$file" ]] || { echo "Missing $dist/$file" >&2; exit 1; }; done
...
for file in magick-core.js magick-core.wasm magick-core.js.gz magick-core.wasm.gz manifest.json features.json imagemagick-config.txt browser-imagemagick.js BUILDINFO.txt; do cp "$dist/$file" "$stage/"; done
...
(cd "$release" && sha256sum "$asset" "$source_name" BUILDINFO-browser-full.txt > SHA256SUMS.txt)
