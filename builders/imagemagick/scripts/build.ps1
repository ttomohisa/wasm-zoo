...
& node (Join-Path $RepoRoot 'scripts\generate-build-metadata.mjs') --slug imagemagick --profile $Profile --dist $out
if($LASTEXITCODE -ne 0){throw "Supply-chain metadata generation failed with exit code $LASTEXITCODE"}
Write-Host "[OK] ImageMagick $Profile build + browser smoke test + provenance/SBOM passed" -ForegroundColor Green
