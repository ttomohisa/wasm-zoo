...
function resolveBrowser() {
  const candidates = [process.env.IMAGEMAGICK_WASM_BROWSER, process.env.CHROME_PATH];
  if (process.platform === "win32") {
...
async function main() {
  ...
  const timeoutMs = Number(process.env.IMAGEMAGICK_WASM_SMOKE_TIMEOUT_MS || 120000);
  ...
  await fsp.copyFile(path.join(root, "runtime", "browser-imagemagick.js"), path.join(dist, "browser-imagemagick.js"));
  ...
  if (!browser) throw new Error("Chromium browser not found. Set IMAGEMAGICK_WASM_BROWSER to Chrome/Edge executable.");
  ...
  const temp = await fsp.mkdtemp(path.join(os.tmpdir(), "wasm-zoo-imagemagick-"));
