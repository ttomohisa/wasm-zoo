/* WASM Zoo ImageMagick browser runtime. Upstream magick CLI wrapper. */
(() => {
  "use strict";
  const TOOL = "magick";
  const WORKER_SOURCE = String.raw`
    ...
    const postFatal = (error, fallback = "ImageMagick worker failed.", files = []) => {
    ...
    self.addEventListener("error", (event) => {
      postFatal(event.error, event.message || "ImageMagick worker failed to initialize.");
      event.preventDefault();
    });
    self.addEventListener("unhandledrejection", (event) => {
      postFatal(event.reason, "ImageMagick worker promise rejected unexpectedly.");
      event.preventDefault();
    });
    ...
    self.onmessage = async (event) => {
      ...
      try {
        ...
        if (typeof createImageMagickCore !== "function") throw new Error("createImageMagickCore factory was not found.");
        core = await createImageMagickCore({
        ...
        if (exitCode !== 0) {
          ...
          throw new Error("ImageMagick CLI exited with code " + exitCode + detail);
        }
        ...
        const resultFiles = collectRequestedFiles();
        postFatal(error, "ImageMagick worker failed.", resultFiles);
      }
    };
  `;
  ...
  const assertSupported = () => {
    if (typeof Worker === "undefined" || typeof WebAssembly === "undefined") {
      throw new Error("WASM Zoo ImageMagick browser-full requires Web Workers and WebAssembly support.");
    }
  };
  ...
  class ImageMagickRunner {
    ...
    async exec(args = [], options = {}) {
      if (this.disposed) throw new Error("ImageMagick runner has been disposed.");
      assertSupported();
      ...
      return new Promise((resolve, reject) => {
        ...
        if (Number.isFinite(timeoutMs) && timeoutMs > 0) {
          timer = setTimeout(() => {
            const error = new Error(`ImageMagick CLI timed out after ${timeoutMs} ms`);
        ...
        worker.onmessage = (event) => {
          ...
          if (message?.type === "error") {
            const error = new Error(message.message || "ImageMagick worker failed.");
