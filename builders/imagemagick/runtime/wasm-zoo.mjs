// WASM Zoo Consumer API v1 ESM adapter for ImageMagick.
...
export const packageInfo = Object.freeze({
  ...
  classicScript: "browser-imagemagick.js",
...
function legacyApi() { return globalThis.WasmZooImageMagick; }

async function ensureLegacy(baseUrl) {
  if (legacyApi()) return legacyApi();
  if (typeof document === "undefined") throw new Error("ImageMagick Consumer API requires a browser document to load browser-imagemagick.js.");
  if (!classicPromise) {
    ...
    classicPromise = new Promise((resolve, reject) => {
      ...
      script.async = true;
      script.onload = () => legacyApi() ? resolve(legacyApi()) : reject(new Error("browser-imagemagick.js loaded without exposing WasmZooImageMagick."));
      script.onerror = () => reject(new Error(`Failed to load ImageMagick legacy runtime: ${scriptUrl}`));
...
function createRuntime(runner, profile) {
  ...
  return Object.freeze({
    ...
    async exec(args = [], options = {}) {
      if (disposed) throw new Error("ImageMagick Consumer API runtime has been disposed.");
...
export async function load(options = {}) {
  if (!isSupported()) throw new Error("ImageMagick Consumer API is not supported in this browser context. Required: " + packageInfo.requires.join(", ") + ".");
