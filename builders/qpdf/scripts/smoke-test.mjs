...
function resolveBrowser() {
  const candidates = [process.env.QPDF_WASM_BROWSER, process.env.CHROME_PATH];
  if (process.platform === "win32") {
...
async function main() {
  ...
  const timeoutMs = Number(process.env.QPDF_WASM_SMOKE_TIMEOUT_MS || 240000);
  ...
  await fsp.copyFile(path.join(root, "runtime", "browser-qpdf.js"), path.join(dist, "browser-qpdf.js"));
  ...
  if (!browser) throw new Error("Chromium browser not found. Set QPDF_WASM_BROWSER to Chrome/Edge executable.");
  ...
  const temp = await fsp.mkdtemp(path.join(os.tmpdir(), "wasm-zoo-qpdf-"));
