...
for file in browser-qpdf.js wasm-zoo.mjs qpdf-core.js qpdf-core.wasm manifest.json features.json provenance.json sbom.cdx.json qpdf-config.txt BUILDINFO.txt LICENSE-QPDF.txt NOTICE-QPDF.md; do [[ -s "$DIST/$file" ]] || { echo "Missing release input: $file" >&2; exit 1; }; done
...
cp "$DIST/LICENSE-QPDF.txt" "$stage/LICENSES/QPDF-LICENSE.txt"; cp "$DIST/NOTICE-QPDF.md" "$stage/LICENSES/QPDF-NOTICE.md"
...
printf '[OK] QPDF experimental release assets prepared locally in %s (no publication performed)\n' "$RELEASE"
