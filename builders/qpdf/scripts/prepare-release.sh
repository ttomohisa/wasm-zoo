...
for cmd in curl tar gzip sha256sum sha512sum zip; do command -v "$cmd" >/dev/null || { echo "Missing release tool: $cmd" >&2; exit 1; }; done

PROFILE="browser-full"
DIST="$ROOT/dist/$PROFILE"
RELEASE="$ROOT/release"
for file in browser-qpdf.js wasm-zoo.mjs qpdf-core.js qpdf-core.wasm manifest.json features.json provenance.json sbom.cdx.json qpdf-config.txt BUILDINFO.txt LICENSE-QPDF.txt NOTICE-QPDF.md; do
...
zlib_archive="$work/zlib-$ZLIB_PORT_VERSION.tar.gz"
jpeg_archive="$work/jpeg-$LIBJPEG_PORT_VERSION.tar.gz"
...
zlib $ZLIB_PORT_VERSION: zlib license; see zlib-LICENSE.txt.
Independent JPEG Group libjpeg $LIBJPEG_PORT_VERSION: IJG terms; see libjpeg-README.txt.
...
mkdir -p "$source_root/qpdf-$QPDF_VERSION" "$source_root/zlib-$ZLIB_PORT_VERSION" "$source_root/libjpeg-$LIBJPEG_PORT_VERSION" "$source_root/emscripten-port-recipes" "$source_root/wasm-zoo-builder"
tar -xzf "$qpdf_archive" --strip-components=1 -C "$source_root/qpdf-$QPDF_VERSION"
cp -a "$zlib_src/." "$source_root/zlib-$ZLIB_PORT_VERSION/"
cp -a "$jpeg_src/." "$source_root/libjpeg-$LIBJPEG_PORT_VERSION/"
...
grep -Fq "VERSION = '$ZLIB_PORT_VERSION'" "$source_root/emscripten-port-recipes/tools/ports/zlib.py"
grep -Fq "HASH = '$ZLIB_SOURCE_SHA512'" "$source_root/emscripten-port-recipes/tools/ports/zlib.py"
grep -Fq "VERSION = '$LIBJPEG_PORT_VERSION'" "$source_root/emscripten-port-recipes/tools/ports/libjpeg.py"
