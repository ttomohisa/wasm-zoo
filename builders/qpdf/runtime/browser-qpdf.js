/* WASM Zoo QPDF browser runtime. Upstream qpdf CLI wrapper. */
(() => {
  "use strict";
  const WORKER_SOURCE = String.raw`
    "use strict";
    let fatalPosted = false;
    const postFatal = (error, fallback = "qpdf worker failed.", files = []) => {
    ...
    self.addEventListener("error", (event) => { postFatal(event.error, event.message || "qpdf worker failed to initialize."); event.preventDefault(); });
    self.addEventListener("unhandledrejection", (event) => { postFatal(event.reason, "qpdf worker promise rejected unexpectedly."); event.preventDefault(); });
    ...
    self.onmessage = async (event) => {
      ...
      } catch (error) { postFatal(error, "qpdf worker failed.", collectRequestedFiles()); }
    };
  `;
  ...
  const assertSupported = () => { if (typeof Worker === "undefined" || typeof WebAssembly === "undefined") throw new Error("WASM Zoo QPDF browser-full requires Web Workers and WebAssembly support."); };
  ...
  class QpdfRunner {
    constructor({ baseUrl, coreJsUrl, wasmUrl }) {
      ...
      this.coreJsUrl = new URL(coreJsUrl || "qpdf-core.js", this.baseUrl).href;
      this.wasmUrl = new URL(wasmUrl || "qpdf-core.wasm", this.baseUrl).href;
      this.wasm = null;
      this.disposed = false;
    }
    async load() { assertSupported(); if (this.wasm) return; const response = await fetch(this.wasmUrl); if (!response.ok) throw new Error(`Failed to load QPDF WASM: ${response.status} ${response.statusText}`); this.wasm = await response.arrayBuffer(); }
    async exec(args = [], options = {}) {
      if (this.disposed) throw new Error("qpdf runner has been disposed."); assertSupported(); if (!Array.isArray(args)) throw new TypeError("args must be an array of CLI arguments."); await this.load();
      ...
      return new Promise((resolve, reject) => {
        ...
        if (Number.isFinite(timeoutMs) && timeoutMs > 0) timer = setTimeout(() => { const error = new Error(`qpdf CLI timed out after ${timeoutMs} ms`); error.name = "TimeoutError"; rejectOnce(error); }, timeoutMs);
        worker.onmessage = (event) => { const message = event.data; if (message?.type === "log") { onLog({ stream: message.stream, message: message.message }); return; }
          ...
          if (message?.type === "error") { const error = new Error(message.message || "qpdf worker failed."); error.name = message.name || "Error"; error.stack = message.stack || error.stack; error.files = Array.isArray(message.files) ? message.files.map((file) => ({ name: file.name, data: new Uint8Array(file.data) })) : []; rejectOnce(error); }
        };
        worker.onerror = (event) => rejectOnce(event.error instanceof Error ? event.error : new Error(event.message || "qpdf worker failed."));
  ...
  window.WasmZooQpdf = Object.freeze({ loadHosted: ({ baseUrl, coreJsUrl, wasmUrl }) => new QpdfRunner({ baseUrl, coreJsUrl, wasmUrl }), isSupported: () => typeof Worker !== "undefined" && typeof WebAssembly !== "undefined" });
})();
