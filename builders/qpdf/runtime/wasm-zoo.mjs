...
function legacyApi() { return globalThis.WasmZooQpdf; }

async function ensureLegacy(baseUrl) {
  if (legacyApi()) return legacyApi();
  if (typeof document === "undefined") throw new Error("QPDF Consumer API requires a browser document to load browser-qpdf.js.");
  if (!classicPromise) {
    ...
    classicPromise = new Promise((resolve, reject) => {
      ...
      script.async = true;
      script.onload = () => legacyApi() ? resolve(legacyApi()) : reject(new Error("browser-qpdf.js loaded without exposing WasmZooQpdf."));
      script.onerror = () => reject(new Error(`Failed to load QPDF legacy runtime: ${scriptUrl}`));
...
function createRuntime(runner, profile) {
  ...
  return Object.freeze({
    ...
    async exec(args = [], options = {}) {
      if (disposed) throw new Error("QPDF Consumer API runtime has been disposed.");
...
export async function load(options = {}) {
  if (!isSupported()) throw new Error("QPDF Consumer API is not supported in this browser context. Required: " + packageInfo.requires.join(", ") + ".");
