...
const pageScopeUrl = new URL('./', location.href).href;
const legacyPlaygroundScopeUrl = new URL('../playground/', location.href).href;
...
async function unregisterLegacyPlaygroundWorkers() {
  const registrations = await navigator.serviceWorker.getRegistrations();
  let removed = false;
  for (const registration of registrations) {
    const scope = normalize(registration.scope);
    if (scope !== pageScopeUrl && scope !== legacyPlaygroundScopeUrl) continue;
...
async function ensureRootIsolationWorker() {
  const removedLegacy = await unregisterLegacyPlaygroundWorkers();
...
if (globalThis.crossOriginIsolated && typeof SharedArrayBuffer !== 'undefined') {
  // Even if the document is already isolated by the legacy Playground-scoped worker,
